...
class TestPluginList:
    ...
    def test_list_empty_plugins_dir(self, tmp_path):
        ...
        with patch.object(matrix_cli, "PLUGINS_DIR", empty_dir), patch.object(matrix_cli, "CONFIG_PATH", empty_cfg):
            result = CliRunner().invoke(matrix_cli.cli, ["plugin", "list"])
        assert result.exit_code == 0
    ...
    def test_list_shows_dev_link_for_symlink(self, tmp_path, fake_config_path):
        ...
        real_repo.mkdir()
        (real_repo / "manifest.json").write_text(
            json.dumps(
                {
                    "id": "dev-plugin",
                    "name": "Dev Plugin",
        ...
        with (
            patch.object(matrix_cli, "PLUGINS_DIR", plugins_dir),
            patch.object(matrix_cli, "CONFIG_PATH", fake_config_path),
        ):
            result = CliRunner().invoke(matrix_cli.cli, ["plugin", "list"])
...
class TestPluginEnableDisable:
    ...
    def test_enable_preserves_other_config_keys(self, tmp_path):
        cfg_path = tmp_path / "config.json"
        cfg_path.write_text(json.dumps({"plugin-a": {"enabled": True, "display_duration": 30}}))
        with patch.object(matrix_cli, "CONFIG_PATH", cfg_path):
...
class TestPluginNew:
    ...
    def test_new_creates_required_files(self, tmp_path):
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "my-plugin", "--path", str(tmp_path), "--no-interactive"]
            )
    ...
    def test_new_manifest_has_correct_id(self, tmp_path):
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "test-id", "--path", str(tmp_path), "--no-interactive"]
            )
    ...
    def test_new_manager_contains_correct_class_name(self, tmp_path):
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "my-widget", "--path", str(tmp_path), "--no-interactive"]
            )
    ...
    def test_new_config_schema_is_valid_json(self, tmp_path):
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "json-test", "--path", str(tmp_path), "--no-interactive"]
            )
    ...
    def test_new_calls_git_init(self, tmp_path):
        ...
        with patch("subprocess.run", return_value=mock_proc) as mock_sub:
            CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "git-plugin", "--path", str(tmp_path), "--no-interactive"]
            )
    ...
    def test_new_fails_if_directory_already_exists(self, tmp_path):
        ...
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            result = CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "existing-plugin", "--path", str(tmp_path), "--no-interactive"]
            )
        assert result.exit_code != 0
    ...
    def test_new_prints_plugin_summary(self, tmp_path):
        with patch("subprocess.run", return_value=MagicMock(returncode=0)):
            result = CliRunner().invoke(
                matrix_cli.cli, ["plugin", "new", "summary-plugin", "--path", str(tmp_path), "--no-interactive"]
            )
...
class TestPluginLinkUnlink:
    ...
    def test_link_calls_dev_setup_with_link_args(self, tmp_path):
        ...
        fake_script.touch()
        with patch.object(matrix_cli, "DEV_SETUP", fake_script), patch("matrix_cli._run", return_value=0) as mock_run:
...
class TestPluginInstall:
    ...
    def test_install_by_id_posts_to_install_endpoint(self):
        ...
        with patch("matrix_cli._detect_web", return_value=True), patch("matrix_cli.requests") as r:
            r.post.return_value = resp
...
class TestPluginUninstall:
    ...
    def test_uninstall_prompts_without_yes_flag(self):
        with patch("matrix_cli._detect_web", return_value=True), patch("matrix_cli.requests") as r:
            ...
            CliRunner().invoke(matrix_cli.cli, ["plugin", "uninstall", "clock-simple"], input="n\n")
    ...
    def test_uninstall_skips_prompt_with_yes_flag(self):
        ...
        with patch("matrix_cli._detect_web", return_value=True), patch("matrix_cli.requests") as r:
            r.post.return_value = resp
            result = CliRunner().invoke(matrix_cli.cli, ["plugin", "uninstall", "clock-simple", "--yes"])
    ...
    def test_uninstall_keep_config_sets_preserve_flag(self):
        ...
        with patch("matrix_cli._detect_web", return_value=True), patch("matrix_cli.requests") as r:
            r.post.return_value = resp
            CliRunner().invoke(matrix_cli.cli, ["plugin", "uninstall", "my-plugin", "--yes", "--keep-config"])
        payload = r.post.call_args[1].get("json", {})
        assert payload.get("preserve_config") is True

    def test_uninstall_exits_1_if_web_not_running(self):
        with patch("matrix_cli._detect_web", return_value=False):
            result = CliRunner().invoke(matrix_cli.cli, ["plugin", "uninstall", "my-plugin", "--yes"])
        assert result.exit_code == 1
...
class TestPluginUpdate:
    ...
    def test_update_all_plugins_posts_once_per_plugin(self, fake_plugins_dir):
        ...
        resp.json.return_value = {"success": True}
        with (
            patch("matrix_cli._detect_web", return_value=True),
            patch.object(matrix_cli, "PLUGINS_DIR", fake_plugins_dir),
...
class TestPluginHealth:
    ...
    def test_health_shows_running_state(self):
        resp = MagicMock()
        resp.json.return_value = {
            "plugins": [{"plugin_id": "clock-simple", "state": "running", "error_count": 0, "last_error": None}]
        }
        with patch("matrix_cli._detect_web", return_value=True), patch("matrix_cli.requests") as r:
            r.get.return_value = resp
            result = CliRunner().invoke(matrix_cli.cli, ["plugin", "health"])
...
class TestPluginStore:
    ...
    def test_store_fetches_from_registry_url(self, fake_plugins_dir):
        ...
        with patch("matrix_cli.requests") as r, patch.object(matrix_cli, "PLUGINS_DIR", fake_plugins_dir):
            r.get.return_value = resp
...
class TestSetupCommand:
    ...
    def test_setup_success(self):
        mock_proc = MagicMock(returncode=0)
        with patch("shutil.which", return_value="/usr/bin/uv"), patch("subprocess.run", return_value=mock_proc):
...
class TestDoctorCommand:
    ...
    def _base_patches(self, tmp_path, venv_exists=True, config_exists=True, secrets_exists=False, plugins_count=1):
    ...
    def test_doctor_rgbmatrix_warn_on_pi_hardware(self, tmp_path):
        ...
        with (
            ...
            patch("shutil.which", return_value="/usr/bin/uv"),
            patch.object(
                Path, "exists", autospec=True, side_effect=self._make_path_exists_with_dev_mem(original_exists)
            ),
