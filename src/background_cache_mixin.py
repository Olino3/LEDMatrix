...
class BackgroundCacheMixin:
            ...
            if live_manager_class and isinstance(self, live_manager_class):
                ...
                if self.cache_manager.is_background_data_available(cache_key, sport_key):
                    ...
                    if cached_data:
                        ...
                        self.logger.warning(
                            f"[{sport_key.upper()}] Background cache check passed but no data returned for {cache_key}"
                        )
                        result = None
    ...
    def _log_fetch_performance(self, sport_key: str, duration: float, cache_hit: bool, cache_source: str):
        ...
        self.logger.info(
            f"[{sport_key.upper()}] Fetch completed in {duration:.2f}s (cache_hit={cache_hit}, source={cache_source})"
        )
        ...
        if self._fetch_count % 10 == 0:
            ...
            self.logger.info(
                f"[{sport_key.upper()}] Cache Performance Summary - "
