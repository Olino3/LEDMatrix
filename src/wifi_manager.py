...
class WiFiManager:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        logger.info(
            f"WiFi Manager initialized - nmcli: {self.has_nmcli}, iwlist: {self.has_iwlist}, "
    ...
    def _show_led_message(self, message: str, duration: int = 5):
        ...
        try:
            ...
            status = {"message": message, "timestamp": time.time(), "duration": duration}
    ...
    def _check_command(self, command: str) -> bool:
        ...
        try:
            ...
            sbin_paths = [f"/usr/sbin/{command}", f"/sbin/{command}", f"/usr/local/sbin/{command}"]
        ...
        try:
            if self.has_nmcli:
                ...
                result = subprocess.run(
                    ["nmcli", "-t", "-f", "DEVICE,TYPE", "device", "status"], capture_output=True, text=True, timeout=5
                )
                if result.returncode == 0:
    ...
    def _get_status_nmcli(self) -> WiFiStatus:
        ...
        try:
            ...
            return WiFiStatus(
                connected=wifi_connected, ssid=ssid, ip_address=ip_address, signal=signal, ap_mode_active=ap_active
            )
        except Exception as e:
    ...
    def _get_status_iwconfig(self) -> WiFiStatus:
        ...
        try:
            result = subprocess.run(["iwconfig", self._wifi_interface], capture_output=True, text=True, timeout=5)
            ...
            return WiFiStatus(connected=connected, ssid=ssid, ip_address=ip_address, ap_mode_active=ap_active)
        except Exception as e:
            ...
            if result.returncode == 0:
                ...
                for line in lines:
                    ...
                    if re.match(r"^\d+:\s+(eth\d+|enp\d+s\d+|enx[0-9a-f]+):", line):
                        in_ethernet = True
                    elif in_ethernet and "inet " in line and "127.0.0.1" not in line:
    ...
    def _is_ap_mode_active(self) -> bool:
        ...
        try:
            ...
            result = subprocess.run(
                ["systemctl", "is-active", HOSTAPD_SERVICE], capture_output=True, text=True, timeout=2
            )
    ...
    def _scan_iwlist(self) -> List[WiFiNetwork]:
        ...
        try:
            result = subprocess.run(
                ["iwlist", self._wifi_interface, "scan"], capture_output=True, text=True, timeout=30
            )
                ...
                if "ESSID:" in line:
                    ...
                    if match:
                        if current_ssid and current_ssid not in seen_ssids:
                            networks.append(
                                WiFiNetwork(ssid=current_ssid, signal=current_signal, security=current_security)
                            )
            ...
            # Add last network
            if current_ssid and current_ssid not in seen_ssids:
                networks.append(WiFiNetwork(ssid=current_ssid, signal=current_signal, security=current_security))
        ...
        try:
            ...
            if self._is_ap_mode_active():
                ...
                if not disable_success:
                    error_msg = (
                        f"Failed to disable AP mode: {disable_msg}. Cannot connect to WiFi while AP mode is active."
                    )
                ...
                for attempt in range(max_verify_attempts):
                    ...
                    if attempt < max_verify_attempts - 1:
                        ...
                        error_msg = (
                            "AP mode disable reported success but AP mode is still active. Cannot connect to WiFi."
                        )
                        logger.error(error_msg)
                        return False, error_msg
            ...
            if original_ssid and original_ssid != ssid:
                ...
                if disconnect_success:
                    ...
                    while wait_count < max_wait:
                        ...
                        if result.returncode == 0:
                            state = (
                                result.stdout.strip().split(":")[-1] if ":" in result.stdout else result.stdout.strip()
                            )
            ...
            if self.has_nmcli:
                ...
                if not success and original_connection and original_ssid:
                    logger.warning(
                        f"Connection to {ssid} failed, attempting to restore original connection: {original_ssid}"
                    )
        ...
        try:
            ...
            if not existing_conn_name:
                direct_check = subprocess.run(
                    ["nmcli", "connection", "show", ssid], capture_output=True, text=True, timeout=5
                )
            ...
            if existing_conn_name:
                ...
                for wait_attempt in range(max_wait):
                    ...
                    if device_result.returncode == 0:
                        state = (
                            device_result.stdout.strip().split(":")[-1]
                            if ":" in device_result.stdout
                            else device_result.stdout.strip()
                        )
                        if state in ["disconnected", "unavailable", "unmanaged"]:
                            break
                ...
                result = subprocess.run(
                    ["nmcli", "connection", "up", existing_conn_name], capture_output=True, text=True, timeout=30
                )
    ...
    def disconnect_from_network(self, skip_ap_check: bool = False) -> Tuple[bool, str]:
        ...
        try:
            ...
            if self.has_nmcli:
                            ...
                            if ":" in line:
                                ...
                                if len(parts) >= 2:
                                    ...
                                    if conn_ssid == status.ssid:
                                        ...
                                        subprocess.run(
                                            ["nmcli", "connection", "down", conn_name], capture_output=True, timeout=10
                                        )
                ...
                result = subprocess.run(
                    ["nmcli", "device", "disconnect", self._wifi_interface], capture_output=True, text=True, timeout=10
                )
    ...
    def _save_network(self, ssid: str, password: str):
        ...
        self.config["saved_networks"] = [n for n in self.config["saved_networks"] if n.get("ssid") != ssid]

        # Add new entry
        self.config["saved_networks"].append({"ssid": ssid, "password": password, "saved_at": time.time()})
    ...
    def _ensure_wifi_radio_enabled(self, max_retries: int = 3) -> bool:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if result.returncode == 0:
                    ...
                    if status == "disabled" or attempt > 0:
                        ...
                        enable_result = subprocess.run(
                            ["sudo", "nmcli", "radio", "wifi", "on"], capture_output=True, text=True, timeout=10
                        )
                ...
                # Fallback: try rfkill
                rfkill_result = subprocess.run(["rfkill", "list", "wifi"], capture_output=True, text=True, timeout=5)
    ...
    def _enable_ap_mode_hostapd(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            try:
                ...
                subprocess.run(
                    ["sudo", "nmcli", "device", "disconnect", self._wifi_interface], capture_output=True, timeout=10
                )

                # Set static IP for AP mode
                subprocess.run(
                    ["sudo", "ip", "addr", "flush", "dev", self._wifi_interface], capture_output=True, timeout=10
                )
                subprocess.run(
                ...
                subprocess.run(
                    ["sudo", "ip", "link", "set", self._wifi_interface, "up"], capture_output=True, timeout=10
                )
            ...
            try:
                ...
                result = subprocess.run(
                    ["sudo", "systemctl", "start", HOSTAPD_SERVICE], capture_output=True, text=True, timeout=15
                )
                if result.returncode != 0:
                ...
                # Start dnsmasq
                result = subprocess.run(
                    ["sudo", "systemctl", "start", DNSMASQ_SERVICE], capture_output=True, text=True, timeout=10
                )
                if result.returncode != 0:
                ...
                try:
                    ...
                    iptables_check = subprocess.run(["which", "iptables"], capture_output=True, timeout=2)

                    if iptables_check.returncode == 0:
                        # Enable IP forwarding (needed for NAT)
                        subprocess.run(
                            ["sudo", "sysctl", "-w", "net.ipv4.ip_forward=1"], capture_output=True, timeout=5
                        )
                        ...
                        logger.info(
                            "Note: Port 80 forwarding requires iptables. Users will need to access port 5000 directly."
                        )
                except Exception as e:
    ...
    def _enable_ap_mode_nmcli_hotspot(self) -> Tuple[bool, str]:
        ...
        try:
                ...
                for line in result.stdout.strip().split("\n"):
                    if ":" in line:
                        ...
                        if len(parts) >= 2:
                            ...
                            should_delete = (
                                "hotspot" in conn_type
                                or conn_ssid == ap_ssid
                                or "hotspot" in conn_name.lower()
                                or conn_name in ["Hotspot", "LEDMatrix-Setup-AP", "TickerSetup-AP"]
                            )

                            if should_delete:
                                logger.info(
                                    f"Deleting existing connection: {conn_name} (type: {conn_type}, SSID: {conn_ssid})"
                                )
                                ...
                                # Then delete it
                                subprocess.run(
                                    ["nmcli", "connection", "delete", conn_name], capture_output=True, timeout=10
                                )
            ...
            for conn_name in ["Hotspot", "LEDMatrix-Setup-AP", "TickerSetup-AP"]:
                ...
                subprocess.run(["nmcli", "connection", "delete", conn_name], capture_output=True, timeout=10)
            ...
            cmd = [
                ...
                ap_ssid,
                "band",
                "bg",  # 2.4GHz for maximum compatibility
                "channel",
                str(ap_channel),
            ...
            if result.returncode == 0:
                ...
                if verify_result.returncode == 0:
                    ...
                    if key_mgmt != "none" or (psk and psk != ""):
                        logger.warning(
                            f"Hotspot still has security (key-mgmt={key_mgmt}, psk={'set' if psk else 'empty'}), deleting and recreating..."
                        )
                        # Delete and recreate as last resort
                        subprocess.run(
                            ["nmcli", "connection", "down", "LEDMatrix-Setup-AP"], capture_output=True, timeout=5
                        )
                        subprocess.run(
                            ["nmcli", "connection", "delete", "LEDMatrix-Setup-AP"], capture_output=True, timeout=5
                        )
                        time.sleep(1)
                ...
                subprocess.run(["nmcli", "connection", "down", "LEDMatrix-Setup-AP"], capture_output=True, timeout=5)
                time.sleep(1)
                subprocess.run(["nmcli", "connection", "up", "LEDMatrix-Setup-AP"], capture_output=True, timeout=10)
        ...
        try:
            ...
            try:
                if hostapd_active:
                    ...
                    subprocess.run(["sudo", "systemctl", "stop", HOSTAPD_SERVICE], capture_output=True, timeout=10)
                    subprocess.run(["sudo", "systemctl", "stop", DNSMASQ_SERVICE], capture_output=True, timeout=10)
                else:
                    # Disable nmcli hotspot mode (fallback)
                    for conn_name in ["LEDMatrix-Setup-AP", "Hotspot", "TickerSetup-AP"]:
                        subprocess.run(["nmcli", "connection", "down", conn_name], capture_output=True, timeout=10)
                        subprocess.run(["nmcli", "connection", "delete", conn_name], capture_output=True, timeout=10)
                ...
                if hostapd_active:
                    ...
                    if os.path.exists(backup_path):
                        subprocess.run(["sudo", "cp", backup_path, str(DNSMASQ_CONFIG_PATH)], timeout=10)
                        ...
                        subprocess.run(["sudo", "cp", "/tmp/dnsmasq.conf", str(DNSMASQ_CONFIG_PATH)], timeout=10)
                ...
                if hostapd_active:
                    try:
                        ...
                        subprocess.run(
                            ["sudo", "sysctl", "-w", "net.ipv4.ip_forward=0"], capture_output=True, timeout=5
                        )
                    ...
                    if not connectivity_safe:
                        ...
                        logger.warning(
                            "No connectivity safety detected (no Ethernet, WiFi may be disabled), ensuring WiFi radio enabled before restart"
                        )
                    ...
                    if not wifi_enabled:
                        logger.warning(
                            "WiFi radio may be disabled after NetworkManager restart - this could cause lockout if Ethernet not connected"
                        )
...
"""
            ...
            subprocess.run(["sudo", "cp", "/tmp/hostapd.conf", str(HOSTAPD_CONFIG_PATH)], timeout=10)
    ...
    def _check_dnsmasq_conflict(self) -> Tuple[bool, str]:
        ...
        try:
            ...
            result = subprocess.run(["systemctl", "is-active", "dnsmasq"], capture_output=True, text=True, timeout=5)
            if result.stdout.strip() == "active":
    ...
    def _create_dnsmasq_config(self):
        ...
        try:
            ...
            if DNSMASQ_CONFIG_PATH.exists():
                subprocess.run(["sudo", "cp", str(DNSMASQ_CONFIG_PATH), f"{DNSMASQ_CONFIG_PATH}.backup"], timeout=10)
...
"""
    ...
    def check_and_manage_ap_mode(self) -> bool:
        ...
        try:
            ...
            logger.debug(
                f"WiFi status: connected={status.connected}, SSID={status.ssid}, "
            ...
            if is_disconnected:
                ...
                logger.debug(
                    f"Network disconnected (check {self._disconnected_checks}/{self._disconnected_checks_required})"
                )
            else:
            ...
            should_have_ap = (
                auto_enable and is_disconnected and self._disconnected_checks >= self._disconnected_checks_required
            )
