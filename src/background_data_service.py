...
class BackgroundDataService:
    ...
    def submit_fetch_request(
        ...
        if cached_data:
            with self._lock:
                ...
                result = FetchResult(request_id=request_id, success=True, data=cached_data, cached=True, fetch_time=0.0)
        ...
        # Submit to executor (fire-and-forget; result handled by worker)
    ...
    def _fetch_data_worker(self, request: FetchRequest) -> FetchResult:
        ...
        try:
            ...
            result = FetchResult(
                request_id=request.id, success=True, data=data, fetch_time=fetch_time, retry_count=request.retry_count
            )
        ...
        for attempt in range(request.max_retries + 1):
            try:
                response = self.session.get(
                    request.url, params=request.params, headers=request.headers, timeout=request.timeout
                )
                return response
        ...
        with self._lock:
            return {
                **self.stats,
                ...
                "completed_requests_usage_percent": (len(self.completed_requests) / self._max_completed_requests * 100)
                if self._max_completed_requests > 0
                else 0,
    ...
    def log_memory_stats(self):
        ...
        logger.info(
            f"BackgroundDataService Memory - Active: {stats['active_requests']}, "
    ...
    def _cleanup_completed_requests(self, force: bool = False) -> int:
        ...
        with self._lock:
            ...
            if len(self.completed_requests) > self._max_completed_requests:
                ...
                sorted_requests = sorted(self.completed_requests.items(), key=lambda x: x[1].completed_at)
            ...
            if removed_count > 0:
                logger.debug(
                    f"Cleaned up {removed_count} old completed requests (remaining: {len(self.completed_requests)})"
                )
