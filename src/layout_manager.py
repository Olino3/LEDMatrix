...
class LayoutManager:
    ...
    def create_preset_layouts(self) -> None:
        ...
        # Basic clock layout
        clock_layout = [
            {"type": "clock", "x": 10, "y": 10, "properties": {"format": "%H:%M", "color": [255, 255, 255]}},
            {"type": "clock", "x": 10, "y": 20, "properties": {"format": "%m/%d", "color": [100, 100, 255]}},
        ]
        self.create_layout("basic_clock", clock_layout, "Simple clock with date")

        # Weather layout
        weather_layout = [
            {"type": "weather_icon", "x": 5, "y": 5, "properties": {"condition": "sunny", "size": 20}},
            {
                "type": "data_text",
                "x": 30,
                "y": 8,
