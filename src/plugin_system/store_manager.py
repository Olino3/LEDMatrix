...
class PluginStoreManager:
        ...
        try:
            ...
            if "github.com" in repo_url:
                ...
                if len(parts) >= 2:
                    ...
                    headers = {"Accept": "application/vnd.github.v3+json", "User-Agent": "LEDMatrix-Plugin-Manager/1.0"}
    ...
    def fetch_registry(self, force_refresh: bool = False) -> Dict:
        ...
        if (
            ...
            and (current_time - self.registry_cache_time) < self.registry_cache_timeout
        ):
            ...
            if repo_url:
                ...
                if fetch_commit_info:
                    ...
                    if commit_info:
                        ...
                        enhanced_plugin["last_updated_iso"] = commit_info.get("date_iso") or enhanced_plugin.get(
                            "last_updated_iso"
                        )
    ...
    def _fetch_manifest_from_github(
        self, repo_url: str, branch: str = "master", manifest_path: str = "manifest.json", force_refresh: bool = False
    ) -> Optional[Dict]:
        """
    ...
    def get_plugin_info(
        self, plugin_id: str, fetch_latest_from_github: bool = True, force_refresh: bool = False
    ) -> Optional[Dict]:
        """
        ...
        if fetch_latest_from_github:
            ...
            if repo_url:
                ...
                plugin_info["last_updated_iso"] = github_info.get(
                    "last_commit_iso", plugin_info.get("last_updated_iso")
                )
                ...
                manifest_rel = f"{plugin_subpath}/manifest.json" if plugin_subpath else "manifest.json"
                github_manifest = self._fetch_manifest_from_github(
                    repo_url, branch, manifest_rel, force_refresh=force_refresh
                )
                if github_manifest:
        ...
        try:
            ...
            try:
                ...
                if manifest_plugin_id != plugin_id:
                    ...
                    if correct_path.exists():
                        ...
                        if not self._safe_remove_directory(correct_path):
                            self.logger.error(
                                f"Failed to remove existing directory {correct_path}, cannot rename plugin"
                            )
                            return False
                ...
                if "entry_point" not in manifest:
                    ...
                    manifest_modified = True
                    self.logger.info(
                        f"Added missing entry_point field to {plugin_id} manifest (defaulted to manager.py)"
                    )
        ...
        try:
            ...
            if plugin_path:
                ...
                if branch_used:
                    ...
                    if branch_used is None:
                        return {"success": False, "error": "Failed to clone or download repository"}
            ...
            if not manifest_path.exists():
                return {
                    ...
                    + (f" at path: {plugin_path}" if plugin_path else ""),
                }
            ...
            if not plugin_id:
                return {"success": False, "error": "No plugin ID found in manifest"}

            # Validate manifest has required fields
            required_fields = ["id", "name", "class_name", "display_modes"]
            missing_fields = [field for field in required_fields if field not in manifest]
            if missing_fields:
                return {"success": False, "error": f"Manifest missing required fields: {', '.join(missing_fields)}"}
            ...
            if validation_errors:
                self.logger.warning(
                    f"Manifest version field validation warnings for {plugin_id}: {', '.join(validation_errors)}"
                )
            ...
            if final_path.exists():
                ...
                if not self._safe_remove_directory(final_path):
                    return {"success": False, "error": f"Failed to remove existing plugin directory: {final_path}"}
            ...
            result = {"success": True, "plugin_id": plugin_id, "name": manifest.get("name")}
            if branch_used:
    ...
    def _install_from_monorepo_api(self, repo_url: str, branch: str, plugin_subpath: str, target_path: Path) -> bool:
        ...
        try:
            ...
            if len(file_entries) > max_files:
                self.logger.error(
                    f"Plugin {plugin_subpath} has {len(file_entries)} files (limit {max_files}), falling back to ZIP"
                )
                return False
            ...
            for entry in file_entries:
                ...
                if not dest_file.resolve().is_relative_to(target_root):
                    self.logger.error(f"Path traversal detected: {entry['path']!r} resolves outside target directory")
                    if target_path.exists():
                        self._safe_remove_directory(target_path)
                    return False
            ...
            with zipfile.ZipFile(tmp_zip_path, "r") as zip_ref:
                ...
                for member in plugin_members:
                    ...
                    if not member_dest.is_relative_to(temp_extract_resolved):
                        self.logger.error(
                            f"Zip-slip detected: member {member!r} resolves outside temp directory, aborting"
                        )
    ...
    def _install_via_download(self, download_url: str, target_path: Path) -> bool:
        ...
        try:
            ...
            response = self._http_get_with_retries(
                download_url, timeout=60, stream=True, headers={"User-Agent": "LEDMatrix-Plugin-Manager/1.0"}
            )
            response.raise_for_status()
    ...
    def _get_local_git_info(self, plugin_path: Path) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            result = {"sha": sha, "short_sha": sha[:7] if sha else "", "branch": branch}
    ...
    def update_plugin(self, plugin_id: str) -> bool:
        ...
        try:
            ...
            if metadata_path.exists():
                try:
                    ...
                    if metadata.get("install_type") == "bundled":
                        self.logger.info(
                            f"Plugin {plugin_id} is a bundled plugin; updates are delivered via LEDMatrix itself"
                        )
                        return True
            ...
            if git_info:
                ...
                if plugin_info_remote:
                    ...
                    if (
                        local_remote
                        and registry_repo
                        and self._normalize_repo_url(local_remote) != self._normalize_repo_url(registry_repo)
                    ):
                        self.logger.info(
                ...
                try:
                    ...
                    if fetch_result.returncode != 0:
                        self.logger.warning(
                            f"Git fetch failed for {plugin_id}: {fetch_result.stderr or fetch_result.stdout}. Will still attempt pull."
                        )
                    else:
                    ...
                    if checkout_result.returncode != 0:
                        self.logger.warning(
                            f"Git checkout to {local_branch} failed for {plugin_id}: {checkout_result.stderr or checkout_result.stdout}. Will still attempt pull."
                        )
                    ...
                    try:
                        ...
                        if remaining_untracked:
                            self.logger.info(
                                f"Found {len(remaining_untracked)} untracked files in {plugin_id}, will stash them"
                            )
                            has_changes = True
                    except subprocess.TimeoutExpired:
                        ...
                        try:
                            ...
                            stash_result = subprocess.run(
                                [
                                    ...
                                    "-u",
                                    "-m",
                                    f"LEDMatrix auto-stash before update {plugin_id}",
                                ],
                                capture_output=True,
                                text=True,
                                timeout=30,
                            ...
                            if stash_result.returncode == 0:
                                ...
                                self.logger.warning(
                                    f"Failed to stash local changes for {plugin_id}: {stash_result.stderr}"
                                )
                        except subprocess.TimeoutExpired:
                    ...
                    if "would be overwritten" in error_output or "local changes" in error_lower:
                        self.logger.warning(
                            f"Plugin {plugin_id} has local changes that prevent update. Consider committing or stashing changes manually."
                        )
                    elif "refusing to merge unrelated histories" in error_lower:
                        self.logger.error(
                            f"Plugin {plugin_id} has unrelated git histories. Plugin may need to be reinstalled."
                        )
                    elif "authentication" in error_lower or "permission denied" in error_lower:
                        self.logger.error(
                            f"Authentication failed for {plugin_id}. Check git credentials or repository permissions."
                        )
                    elif "not found" in error_lower or "does not exist" in error_lower:
                        self.logger.error(
                            f"Remote branch or repository not found for {plugin_id}. Check repository URL and branch name."
                        )
                    elif "merge conflict" in error_lower or "conflict" in error_lower:
                        self.logger.error(
                            f"Merge conflict detected for {plugin_id}. Resolve conflicts manually or reinstall plugin."
                        )
            ...
            if not plugin_info_remote and repo_url:
                self.logger.info(
                    f"Plugin {plugin_id} not in registry but has git remote URL. Reinstalling from {repo_url} to enable updates..."
                )
                try:
            ...
            if not plugin_info_remote:
                self.logger.warning(
                    f"Plugin {plugin_id} not found in registry and not a git repository; cannot update automatically"
                )
                if not repo_url:
                    self.logger.warning(
                        "Plugin may have been installed via ZIP download. Try reinstalling from GitHub URL to enable updates."
                    )
                return False
