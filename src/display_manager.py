...
class DisplayManager:
    ...
    def _setup_matrix(self):
        ...
        try:
            ...
            logger.info(
                f"Initializing RGB Matrix with settings: rows={options.rows}, cols={options.cols}, chain_length={options.chain_length}, parallel={options.parallel}, hardware_mapping={options.hardware_mapping}"
            )
            ...
            try:
                ...
                pass
            logger.error(
                f"Matrix initialization failed, using fallback mode with size {fallback_width}x{fallback_height}. Error: {e}"
            )
    ...
    def draw_sun(self, x: int, y: int, size: int = 16):
        ...
        self.draw.ellipse(
            [center[0] - radius, center[1] - radius, center[0] + radius, center[1] + radius], fill=(255, 255, 0)
        )  # Yellow
    ...
    def draw_cloud(self, x: int, y: int, size: int = 16, color=(200, 200, 200)):
        ...
        self.draw.ellipse(
            [x + size // 4, y + size // 3, x + size // 4 + size // 2, y + size // 3 + size // 2], fill=color
        )
        self.draw.ellipse(
            [x + size // 2, y + size // 3, x + size // 2 + size // 2, y + size // 3 + size // 2], fill=color
        )
        self.draw.ellipse(
            [x + size // 3, y + size // 6, x + size // 3 + size // 2, y + size // 6 + size // 2], fill=color
        )
    ...
    def _draw_cloud(self, x: int, y: int, size: int) -> None:
        ...
        for cx, cy in positions:
            self.draw.ellipse(
                [cx - circle_radius, cy - circle_radius, cx + circle_radius, cy + circle_radius], fill=cloud_color
            )
    ...
    def is_currently_scrolling(self) -> bool:
        ...
        if (
            current_time - self._scrolling_state["last_scroll_activity"]
            > self._scrolling_state["scroll_inactivity_threshold"]
        ):
    ...
    def defer_update(self, update_func, priority: int = 0):
        ...
        self._scrolling_state["deferred_updates"].append(
            {"func": update_func, "priority": priority, "timestamp": current_time}
        )
    ...
    def process_deferred_updates(self):
        ...
        logger.debug(
            f"Processing {len(updates_to_process)} deferred updates (queue size: {len(self._scrolling_state['deferred_updates'])})"
        )
    ...
    def _cleanup_expired_deferred_updates(self, current_time: float):
        ...
        self._scrolling_state["deferred_updates"] = [
            update for update in self._scrolling_state["deferred_updates"] if current_time - update["timestamp"] <= ttl
        ]
