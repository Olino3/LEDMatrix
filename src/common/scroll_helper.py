...
class ScrollHelper:
    ...
    def __init__(self, display_width: int, display_height: int, logger: Optional[logging.Logger] = None):
        """
    ...
    def create_scrolling_image(self, content_items: list, item_gap: int = 32, element_gap: int = 16) -> Image.Image:
        """
        ...
        if actual_image_width != total_width:
            self.logger.warning(
                ...
                total_width,
                actual_image_width,
                abs(actual_image_width - total_width),
            )
    ...
    def update_scroll_position(self) -> None:
        ...
        if self.frame_based_scrolling:
            ...
            if time_since_last_step >= self.scroll_delay:
                ...
                max_steps = max(
                    1, int(0.04 / self.scroll_delay)
                )  # Limit to 0.04s (2 steps at 50 FPS) for smoother scrolling
        ...
        if is_complete:
            ...
            if not self.scroll_complete:
                ...
                scroll_percent = (
                    (self.total_distance_scrolled / required_total_distance * 100)
                    if required_total_distance > 0
                    else 0.0
                )
                position_percent = (
                    (self.scroll_position / self.total_scroll_width * 100) if self.total_scroll_width > 0 else 0.0
                )
                self.logger.info(
            ...
            if self.scroll_position >= self.total_scroll_width:
                ...
                self.logger.debug(
                    "Clamped scroll position to %d (max=%d)", self.scroll_position, self.total_scroll_width - 1
                )
        else:
            self.scroll_complete = False
    ...
    def _get_visible_portion_integer(self, start_x: int, end_x: int) -> Image.Image:
        ...
        if end_x <= self.cached_image.width:
            ...
            if width1 > 0:
                ...
                if self._frame_buffer is None or self._frame_buffer.shape != (
                    self.display_height,
                    self.display_width,
                    3,
                ):
                ...
                if remaining_width > 0:
                    ...
                    if HAS_SCIPY:
                        ...
                        if shifted2.shape[1] >= remaining_width:
                            self._frame_buffer[:, width1 : width1 + remaining_width] = shifted2[
                                :, :remaining_width
                            ].astype(np.uint8)
                        else:
                            ...
                            if actual_width < remaining_width:
                                self._frame_buffer[:, width1 + actual_width : width1 + remaining_width] = shifted2[
                                    :, -1:
                                ].astype(np.uint8)
                    else:
                        ...
                        if interpolated2.shape[1] == remaining_width:
                            ...
                            if copy_width < remaining_width:
                                self._frame_buffer[:, width1 + copy_width : width1 + remaining_width] = interpolated2[
                                    :, -1:
                                ]
            else:
    ...
    def _interpolate_subpixel(
        self, source: np.ndarray, fractional: float, output_width: Optional[int] = None
    ) -> np.ndarray:
        """
    ...
    def set_scrolling_image(self, image: Image.Image) -> None:
        ...
        self.logger.debug(
            "Set scrolling image: %dx%d, total_scroll_width=%d", image.width, image.height, self.total_scroll_width
        )
    ...
    def set_target_fps(self, fps: float) -> None:
        ...
        self.frame_time_target = 1.0 / self.target_fps
        self.logger.debug(
            f"Target FPS set to: {self.target_fps} FPS (frame_time_target: {self.frame_time_target:.4f}s)"
        )
    ...
    def set_dynamic_duration_settings(
        self, enabled: bool = True, min_duration: int = 30, max_duration: int = 300, buffer: float = 0.1
    ) -> None:
        """
        ...
        self.logger.debug(
            f"Dynamic duration settings: enabled={enabled}, "
    ...
    def log_frame_rate(self) -> None:
        ...
        if current_time - self.last_fps_log_time >= 5.0:
            ...
            self.logger.info(
                f"Scroll frame stats - Avg FPS: {avg_fps:.1f}, "
    ...
    def get_scroll_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "elapsed_time": (time.time() - self.scroll_start_time) if self.scroll_start_time else None,
