...
class DynamicTeamResolver:
    ...
    def resolve_teams(self, team_list: List[str], sport: str = "ncaa_fb") -> List[str]:
        ...
        for team in team_list:
            if team in self.DYNAMIC_PATTERNS:
                ...
                self.logger.info(
                    f"Resolved {team} to {len(dynamic_teams)} teams: {dynamic_teams[:5]}{'...' if len(dynamic_teams) > 5 else ''}"
                )
    ...
    def _fetch_ncaa_fb_rankings(self) -> Dict[str, int]:
        ...
        # Check cache first
        if self._rankings_cache and current_time - self._cache_timestamp < self._cache_duration:
