...
class LogoDownloader:
    ...
    def fetch_teams_data(self, league: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = self.session.get(
                api_url, params={"limit": 1000}, headers=self.headers, timeout=self.request_timeout
            )
            response.raise_for_status()
            data = response.json()
    ...
    def _determine_ncaa_football_division(self, team_info: Dict, league_data: Dict) -> str:
        ...
        known_fbs_teams = {"ASU", "ARIZ", "ARK", "AUB", "BOIS", "CSU", "FLA", "HAW", "IDHO", "USA"}
    ...
    def download_all_ncaa_football_logos(
        self, include_fcs: bool = True, force_download: bool = False
    ) -> Tuple[int, int]:
        ...
        logger.info(
            f"Comprehensive NCAA football logo download complete: {downloaded_count} downloaded, {failed_count} failed"
        )
        return downloaded_count, failed_count

    def download_missing_logo_for_team(
        self, league: str, team_id: str, team_abbreviation: str, logo_path: Path
    ) -> bool:
    ...
    def download_all_missing_logos(
        self, leagues: List[str] | None = None, force_download: bool = False
